...
# ----------------------------------------------------------------------------
# 2026-05-13 实验 #4：2P EV 重平衡
# ----------------------------------------------------------------------------
# V3 训练于 4 人临海（3 个对手），houjuu_prob 的惩罚系数 β 是按"3 家可放炮"
# 校准的。在 2P 实际只 1 个对手，真实放炮代价 ≈ 1/3 训练时代价。所以 V3 的
# candidate.total_ev 把高 houjuu 候选过度压低了；本函数加回
#   new_score = total_ev + factor * houjuu_prob
# 让原本被防守逻辑过度惩罚的候选回升，配合 #3 deep profile 后还能再吃 1–3 pp。
#
# 与既有 adjust_for_2p_linhai 的区别：
#   adjust_for_2p_linhai = 多类启发式（字牌、缺一门、betaori 等），实测净负
#   rebalance_for_2p_houjuu = 仅做线性 EV 修正，无启发式，可 env var 调强度
# 因此本函数走独立路径，不经 adjust_for_2p_linhai。
def rebalance_for_2p_houjuu(
    result: Optional[Dict],
    factor: float,
) -> Optional[Dict]:
    """按 V3 输出的 per-candidate houjuu_prob 重排候选，返回新 best tile。

    依赖 result["candidate_details"]（v3_service.py:#4 起注入）。若不在则直接返回。

    Args:
        result: V3SearchService.recommend_discard() 返回的 dict，含 candidate_details。
        factor: > 0 启用 rebalance；典型值 500–3000，按 candidate_scores 量级调。
                每个候选的 score 加 `factor * houjuu_prob`（houjuu_prob 是 0–1 概率），
                高 houjuu 候选回升 factor * houjuu_prob 分。

    Returns:
        浅 copy 后的 result，candidate_scores / tile / chosen_by 已更新；
        无 factor 或无 candidate_details 时原样返回。
    """
    if not result or not isinstance(result, dict) or factor <= 0:
        return result
    ...
    new_scores: Dict[str, float] = {}
    for tile, fields in details.items():
        if not isinstance(fields, dict):
            continue
        try:
            total_ev = float(fields.get("total_ev", 0.0))
            houjuu_prob = float(fields.get("houjuu_prob", 0.0))
        except (TypeError, ValueError):
            continue
        new_scores[tile] = round(total_ev + factor * houjuu_prob, 3)
    ...
    out["candidate_scores"] = new_scores
    out["tile"] = max(new_scores, key=new_scores.get)
    out["chosen_by"] = (result.get("chosen_by") or "search") + f"+rebal{factor:g}"
    return out
