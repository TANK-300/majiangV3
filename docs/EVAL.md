...
**离线门槛** —— `tools/acceptance_25_8.py` 默认 5 seed × 200 局 = 125 个 8-局窗口：
...
**性能门槛** —— `tools/perf_regression.py`：4 个最坏牌型（4 白板 / 复杂副露 / 多抢杠 / 抓冲全开 + 树掉还原）P95 < 800ms。

**真人 dry-run**（spec §6.1.1）：R3 后组织 4-6 人内测，每人 ≥ 24 局；任一窗口 ≤ 0 回 R2 重训。
